...
export interface LiveQueryHandle<T, E = never, ER = never> {
  /** Current live query rows from the nearest React runtime. */
  ...
  /** Ensure all query sources have loaded inside the nearest React runtime. */
  preloadEffect(): Effect.Effect<void, E | QueryEvaluationError | ER>;
  /** Force a fresh source preload and live query evaluation inside the React runtime. */
