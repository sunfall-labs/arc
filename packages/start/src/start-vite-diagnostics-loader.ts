...
): NonNullable<InlineConfig["plugins"]> extends readonly (infer Plugin)[]
  ? readonly Plugin[]
  : readonly unknown[] =>
...
const startDiagnosticsViteServerEffect = (
  ...
  const inlinePlugins = (
    options.start === undefined ? inlineConfig.plugins : removeStartPlugins(inlineConfig.plugins)
  ...
  const configFile =
    ...
    (options.start === undefined ? undefined : false);
...
const startAppGraphDiagnosticsFromModuleEffect = (
  ...
  Effect.gen(function* () {
    ...
    }).pipe(Effect.mapError((cause) => diagnosticsRunnerError(cause.message, cause)));
...
const loadStartAppGraphDiagnosticsRawEffect = (
  ...
  Effect.gen(function* () {
    ...
    return yield* loadStartAppGraphDiagnosticsWithOwnedServerEffect(
