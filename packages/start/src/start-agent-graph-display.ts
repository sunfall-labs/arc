...
export const factString = (node: StartAgentGraphNode, key: string): string | undefined => {
...
export const routeParamNames = (node: StartAgentGraphNode): readonly string[] => {
...
export const inlineList = (values: readonly string[], empty = "none"): string =>
  values.length === 0 ? empty : values.join(", ");

export const formatWire = (wire: Readonly<Record<string, unknown>> | undefined): string =>
  [
...
export const formatBehavior = (behavior: Readonly<Record<string, unknown>> | undefined): string =>
  [
...
export const titleForNode = (node: StartAgentGraphNode): string => {
  switch (node.kind) {
    case "ServerFunction":
...
export const formatRouteSummary = (node: StartAgentGraphNode): readonly string[] => {
...
export const formatActionSummary = (node: StartAgentGraphNode): readonly string[] => [
  `Wire schemas: ${formatWire(factRecord(node, "wire"))}`,
  `Behavior: ${formatBehavior(factRecord(node, "behavior"))}`,
];

export const formatServerFunctionSummary = (node: StartAgentGraphNode): readonly string[] => [
  `Wire schemas: ${formatWire(factRecord(node, "wire"))}`,
];

export const formatFindingSummary = (node: StartAgentGraphNode): readonly string[] => [
  `Issue: ${factString(node, "issue") ?? "unknown"}`,
  `Fix: ${factString(node, "edit") ?? "unknown"}`,
];

export const formatGenericSummary = (node: StartAgentGraphNode): readonly string[] => [
  `Kind: ${node.kind}`,
  `Name: ${node.label}`,
];

export const publicStatus = (node: StartAgentGraphNode): string =>
  node.status === "known" ? "pass" : "needs attention";
