...
const fakeRowId = (key: SQLitePersistenceKey): string => `${key.namespace}\u0000${key.key}`;
...
interface FakePreparedStatementDatabase extends SQLitePreparedStatementDatabase<
  SQLiteStatementRow,
  never,
  never
> {
...
describe("SQLite persistence storage", () => {
  it("persists collection snapshots into namespace/key rows", () => {
    ...
    return Effect.runPromise(
      Effect.gen(function* () {
        ...
        yield* Effect.scoped(
          first.provide(Projects.persistEffect(storage, { key: "projects-cache" })),
        );
        ...
        yield* Effect.scoped(
          second.provide(Projects.restoreEffect(storage, { key: "projects-cache" })),
        );
  ...
  it("adapts SQL statement databases into the persistence driver interface", () => {
    ...
    const storage = makeSQLitePersistenceStorage(makeSQLiteStatementPersistenceDriver(fake), {
      namespace: "workspace:a",
    ...
    return Effect.runPromise(
      Effect.gen(function* () {
        ...
        yield* Effect.sync(() => {
          expect(fake.executed[0]?.[0]).toContain(
  ...
  it("adapts prepare/run/all SQLite clients into statement databases", () => {
    ...
    const storage = makeSQLitePersistenceStorage(
      makeSQLiteStatementPersistenceDriver(
        makeSQLitePreparedStatementDatabase(fake, { cache: true }),
      ),
      {
        namespace: "workspace:a",
        tableName: "collection-snapshots",
    ...
    return Effect.runPromise(
      Effect.gen(function* () {
        ...
        yield* Effect.sync(() =>
          expect(
            fake.memory.row("collection-snapshots", "workspace:a", "projects-cache"),
          ).toMatchObject({
            namespace: "workspace:a",
            key: "projects-cache",
            schemaVersion: 5,
  ...
  it("reports SQLite persistence callback throws in the Effect error channel", async () => {
    ...
    const clockStorage = makeSQLitePersistenceStorage(
      makeSQLiteStatementPersistenceDriver(makeSQLiteMemoryStatementDatabase()),
      {
        now: () => {
          throw nowFailure;
        },
    ...
    await expect(
      Effect.runPromise(toEffect(storage.getItem("projects-cache"))),
    ).rejects.toMatchObject({
      _tag: "EffectInputCallbackError",
      operation: "SQLitePersistence.get",
    });
    await expect(
      Effect.runPromise(toEffect(clockStorage.setItem("projects-cache", "{}"))),
    ).rejects.toMatchObject({
      _tag: "EffectInputCallbackError",
  ...
  it("provides an in-memory statement database for the generated SQLite persistence SQL", () => {
    const memory = makeSQLiteMemoryStatementDatabase();
    const storage = makeSQLitePersistenceStorage(makeSQLiteStatementPersistenceDriver(memory), {
      namespace: "workspace:a",
      tableName: "collection-snapshots",
      schemaVersion: 7,
      now: () => 123,
    ...
    return Effect.runPromise(
      Effect.gen(function* () {
        ...
        yield* Effect.sync(() => {
          expect(
            memory.row("collection-snapshots", "workspace:a", "projects-cache"),
          ).toBeUndefined();
  ...
  it("rejects malformed memory DELETE params before mutating rows", () => {
    ...
    const insertSql =
      'INSERT INTO "collection-snapshots" ("namespace", "key", "schema_version", "value", "updated_at") ' +
      "VALUES (?, ?, ?, ?, ?)";
    const deleteSql = 'DELETE FROM "collection-snapshots" WHERE "namespace" = ? AND "key" = ?';
    memory.execute(insertSql, ["workspace:a", "123", 7, '{"rows":[]}', 123]);
    ...
    Effect.runPromise(
      Effect.gen(function* () {
        ...
        const versionFailure = yield* Effect.flip(
          toEffect(invalidVersionStorage.setItem("projects-cache", '{"collections":[]}')),
        );
        const timestampFailure = yield* Effect.flip(
          toEffect(invalidTimestampStorage.setItem("projects-cache", '{"collections":[]}')),
        );
  ...
  it("rejects malformed direct-driver string fields before table callbacks observe them", () =>
    Effect.runPromise(
      Effect.gen(function* () {
        ...
        const namespaceFailure = yield* Effect.flip(
          toEffect(invalidNamespaceStorage.getItem("projects-cache")),
        );
        const getKeyFailure = yield* Effect.flip(
          toEffect(storage.getItem(123 as unknown as string)),
        );
        const setKeyFailure = yield* Effect.flip(
          toEffect(storage.setItem(123 as unknown as string, "{}")),
        );
        const valueFailure = yield* Effect.flip(
          toEffect(storage.setItem("projects-cache", 123 as unknown as string)),
        );
        const deleteKeyFailure = yield* Effect.flip(
          toEffect(storage.removeItem!(123 as unknown as string)),
        );
