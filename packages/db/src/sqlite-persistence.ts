...
export interface SQLiteStatementDatabase<E = never, R = never> {
  ...
  readonly select: (
    sql: string,
    params?: SQLiteStatementParams,
  ) => EffectInput<ReadonlyArray<SQLiteStatementRow>, E, R>;
}
...
const sqlTableName = (sql: string): string => {
  ...
  if (!match?.[1]) {
    throw new SQLitePersistenceUnsupportedStatement({
      ...
      operation: "tableName",
      guidance:
        "Use SQL generated by makeSQLiteStatementPersistenceDriver or extend the memory adapter parser.",
    });
  }
...
const invalidStatementRow = (
  ...
  new SQLitePersistenceInvalidRow({
    ...
    expected,
    guidance:
      "SQLite statement adapters must return persisted rows with exact string fields and finite numeric timestamp/version fields.",
  });
...
const invalidStatementParams = (
  ...
  new SQLitePersistenceInvalidStatementParams({
    ...
    ...counts,
    guidance:
      "The in-memory SQLite adapter supports generated statements with exact params: string namespace/key/value fields and finite numeric schema_version/updated_at fields.",
  });
  ...
  return {
    ...
    execute: (sql, params) => {
      ...
      throw new SQLitePersistenceUnsupportedStatement({
        ...
        operation: "execute",
        guidance:
          "The in-memory SQLite adapter supports generated CREATE, INSERT, and DELETE statements only.",
      });
    },
    select: (sql, params) => {
      statements.push(params === undefined ? { sql } : { sql, params });
...
>(
  ...
  const cache =
    options.cache === true ? new Map<string, SQLitePreparedStatement<Row, E, R>>() : undefined;

  const prepare = (
    sql: string,
  ): Effect.Effect<SQLitePreparedStatement<Row, E, R>, E | EffectInputCallbackError, R> =>
    Effect.gen(function* () {
  ...
  return {
    ...
    select: (
      sql,
      params,
    ): Effect.Effect<ReadonlyArray<SQLiteStatementRow>, E | EffectInputCallbackError, R> =>
      Effect.flatMap(prepare(sql), (statement) =>
...
export const makeSQLiteStatementPersistenceDriver = <E = never, R = never>(
  ...
  table: (name) => {
    ...
    return {
      ...
      get: (key) =>
        Effect.gen(function* () {
          const rows = yield* runCallback("select", () =>
            database.select(
          ...
          return {
            namespace: yield* statementRowString("namespace", row.namespace),
            key: yield* statementRowString("key", row.key),
            schemaVersion: yield* statementRowNumber(
              "schema_version",
      ...
      delete: (key) =>
        runCallback("execute", () =>
          database.execute(`DELETE FROM ${table} WHERE "namespace" = ? AND "key" = ?`, [
            key.namespace,
            key.key,
          ]),
        ),
...
export const makeSQLitePersistenceStorage = <E = never, R = never>(
  ...
  E | EffectInputCallbackError | SQLitePersistenceInvalidRow | SQLitePersistenceInvalidTableName,
  R
> => {
      ...
      try: () => driver.table(tableName),
        ...
        cause instanceof SQLitePersistenceInvalidTableName
          ...
          : new EffectInputCallbackError({
              ...
              cause,
              guidance:
                "SQLite persistence drivers must synchronously return a table adapter. Effectful work belongs in table ensure/get/upsert/delete callbacks.",
            }),
    });
  ...
  const runStorageCallback = <A>(
    operation: string,
    callback: () => EffectInput<A, E, R>,
  ): Effect.Effect<A, E | EffectInputCallbackError, R> => runCallback(operation, callback);
  ...
  const rowKeyEffect = (
    key: string,
  ): Effect.Effect<SQLitePersistenceKey, SQLitePersistenceInvalidRow> =>
    Effect.gen(function* () {
      return {
