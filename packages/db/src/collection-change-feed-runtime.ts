...
const completeChangeFeedEmission = <A extends object, K extends CollectionKey, E>(
...
const interruptChangeFeedEmission = <A extends object, K extends CollectionKey, E>(
  emission: CollectionChangeFeedDispatch<A, K, E>,
): Effect.Effect<void> =>
  emission.completed
...
>(
  ...
  Effect.gen(function* () {
    ...
    const dispatcher = yield* scopedCollectionChangeFeedDispatcherEffect<
      A,
      K,
    ...
    const shutdownSetup = Effect.all(
      [
        Fiber.interrupt(consumerFiber).pipe(
          Effect.asVoid,
          Effect.catchCause(() => Effect.void),
        ),
    ...
    yield* Effect.acquireRelease(
      collectionChangeFeedInputCallbackEffect<
        CollectionChangeFeedSubscription<FeedError, FeedRequirements>,
        FeedError,
        FeedRequirements
      >(() =>
      ...
      ).pipe(Effect.onExit((exit) => (Exit.isFailure(exit) ? shutdownSetup : Effect.void))),
      (subscription) => {
        const unsubscribe = changeFeedUnsubscribe(subscription);
        return unsubscribe
          ? collectionChangeFeedInputCallbackEffect<void, FeedError, FeedRequirements>(() =>
              unsubscribe(),
            ).pipe(
              Effect.catchCause((cause) =>
                runtime
                  .publishFailure(changeFeedFailureError(cause))
