...
describe("Server contracts", () => {
  ...
  it("exposes a mock layer for runtime dependency injection", () =>
    Effect.runPromise(
      Effect.provide(
        ...
        Server.mockLayer(Server.mock(GetUser, ({ id }) => Effect.succeed({ id, name: "Grace" }))),
      ).pipe(
  ...
  it("validates mock failures against the contract error schema", () => {
    const FailingUser = Server.contract<void, string, { readonly code: string }>(
      "Test.User.mockFailure",
      {
  ...
  it("dispatches local clients through explicit registry snapshots", () => {
    ...
    Server.implement(EchoContract, (input) => Effect.succeed(`global:${input}`));
    ...
    return Effect.runPromise(
      Effect.provideService(
        ...
        Effect.tap((value) => Effect.sync(() => expect(value).toBe("snapshot:atlas"))),
        Effect.asVoid,
      ),
    );
  });
