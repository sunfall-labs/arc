...
export const promiseLikeSyncCallbackError = (
  ...
  new EffectInputCallbackError({
    ...
    cause: new EffectInputPromiseRejected({
      guidance:
        "Sync metadata callbacks must not return Promise-shaped values. Move host Promise work into an Effect.tryPromise(...) Adapter seam before returning metadata.",
    }),
    guidance,
  });
...
export const rejectPlainSyncCallbackValue = <A>(operation: string, value: A, guidance: string): A =>
