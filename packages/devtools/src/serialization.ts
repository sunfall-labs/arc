...
const normalizeSerializationPolicy = (
  ...
  maxStringLength: normalizePolicyBound(policy?.maxStringLength, defaultSerializationPolicy.maxStringLength),
...
const serializeEntries = <A>(
  ...
  for (const value of values) {
    if (index < policy.maxEntries) {
      ...
      serialized.push(truncatedMarker(knownSize === undefined ? 1 : Math.max(0, knownSize - policy.maxEntries)));
      return serialized;
    }
    index += 1;
...
export const toDevtoolsSerializableFingerprint = (
  ...
  stableSerializableFingerprint(toDevtoolsSerializableValue(value, policy));
...
const initialDetachedCopyState = (
  policy?: DevtoolsSerializationPolicy
): DetachedCopyState => ({
...
const copyBoundedBytes = (
  ...
  const alignedLength = byteAlignment <= 1
    ? boundedLength
    : Math.floor(boundedLength / byteAlignment) * byteAlignment;
...
const copyDetachedArrayBuffer = <A>(
  value: A,
  state: DetachedCopyState
): A => {
  try {
    ...
    return rememberDetachedOriginalSize(bytesToArrayBuffer(bytes), bytes.byteLength, originalByteLength) as A;
  } catch (error) {
    return uninspectableObject(error, state.policy) as A;
  }
};

const copyDetachedDataView = <A>(
  value: A,
  state: DetachedCopyState
): A => {
  try {
...
const copyDetachedArrayBufferView = <A>(
  value: A,
  state: DetachedCopyState
): A => {
  try {
    ...
    const bytesPerElement = typeof (value as { readonly BYTES_PER_ELEMENT?: unknown }).BYTES_PER_ELEMENT === "number"
...
const copyDetachedMap = <A>(
  ...
  try {
    ...
    while (index < copied) {
      ...
      copy.set(
        copyDetachedValue(key, childState),
        copyDetachedValue(item, childState)
      );
      index += 1;
    }
...
const copyDetachedValue = <A>(
  ...
  if (typeof value === "number") {
    return Number.isFinite(value)
      ? value
      : objectTag("NonFiniteNumber", String(value)) as A;
  }
...
export const copyRequestTrace = (
  ...
  return {
    ...
    ...(trace.routePlan === undefined ? {} : { routePlan: copyDevtoolsRoutePlan(trace.routePlan, policy) }),
...
export const copyDevtoolsRuntimeEvent = (
  ...
  switch (event._tag) {
    ...
    case "Custom":
      return {
        ...event,
        ...(event.payload === undefined ? {} : { payload: copyDetachedValue(event.payload, state) })
      };
  }
};
