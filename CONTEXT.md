...
- **Collection Runtime Module**: the DB Module behind `Collection.define(...)` that owns runtime store lookup, load/refetch, direct writes, change batches, optimistic mutation execution, event publication, persistence coordination, and collection preload collection.
