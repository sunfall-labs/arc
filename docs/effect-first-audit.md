...
- The current full gate is the Review 197 `pnpm verify` run recorded in
  `docs/architecture-deepening-review.md`: 11 package builds, workspace
  typecheck, public type tests, public API inventory audit, Effect-first audit
  over 405 files, 53 root test files / 1054 tests, package-level verifies,
...
- Review 135 tightened the Start fetch adapter Promise-return allowance while
  ...
  files.
- The current full `pnpm verify` passed after the Review 197 sync metadata
  guardrails, Start action runtime split, Resource key hardening, and DB
  flush-skip normalization: 11 package
  builds, workspace typecheck, type tests,
  public API inventory audit, Effect-first audit over 405
  package/example/config/script/type-test/generated/docs files, 53 root test
  files / 1054 tests, package-level verifies for copyable/source packages,
