...
- Clean Sweep 9 after Review511 found no actionable Effect-first follow-up:
  Core/React/Solid, DB/React-DB/Solid-DB, Start/docs/package, generated
  artifacts, and evidence lanes preserved Effect-returning public Interfaces
  and explicit host-boundary Promise allowances.
- Clean Sweep 10 after Review511 found no actionable Effect-first follow-up:
  Core/React/Solid, DB/React-DB/Solid-DB, Start/docs/package, generated
  artifacts, and evidence lanes preserved Effect-returning public Interfaces
  and explicit host-boundary Promise allowances.
- Clean Sweep 7 after Review510 found no actionable Effect-first follow-up:
  ...
  artifacts, and evidence lanes preserved Effect-returning public Interfaces
  and explicit host-boundary Promise allowances.
- Clean Sweep 6 after Review510 found no actionable Effect-first follow-up:
  Core/React/Solid, DB/React-DB/Solid-DB, Start/docs/package, generated
  artifacts, and evidence lanes preserved Effect-returning public Interfaces
  and explicit host-boundary Promise allowances.
- Clean Sweep 5 after Review510 found no actionable Effect-first follow-up:
  Core/React/Solid, DB/React-DB/Solid-DB, Start/docs/package, generated
  artifacts, and evidence lanes preserved Effect-returning public Interfaces
  and explicit host-boundary Promise allowances.
- Clean Sweep 4 after Review510 found no actionable Effect-first follow-up:
  Core/React/Solid, DB/React-DB/Solid-DB, Start/docs/package, generated
  artifacts, and evidence lanes preserved Effect-returning public Interfaces
  and explicit host-boundary Promise allowances.
- Clean Sweep 3 after Review510 found no actionable Effect-first follow-up:
  Core/React/Solid, DB/React-DB/Solid-DB, Start/docs/package, generated
  artifacts, and evidence lanes preserved Effect-returning public Interfaces
  and explicit host-boundary Promise allowances.
- Clean Sweep 2 after Review510 found no actionable Effect-first follow-up:
  Core/React/Solid, DB/React-DB/Solid-DB, Start/docs/package, generated
  artifacts, and evidence lanes preserved Effect-returning public Interfaces
  and explicit host-boundary Promise allowances.
- Clean Sweep 1 after Review510 found no actionable Effect-first follow-up:
  Core/React/Solid, DB/React-DB/Solid-DB, Start/docs/package, generated
  artifacts, and evidence lanes preserved Effect-returning public Interfaces
  and explicit host-boundary Promise allowances.
...
- Clean Sweep 1 after Review508 found no actionable Effect-first follow-up:
...
- Review493 Prerender Server Release And Current Evidence Policy keeps the
  ...
  release failures through `StartPrerenderError` operation `close-server`, and
  avoids a Promise-shaped cleanup blind spot. Current evidence policy also now
  rejects stale older full-gate claims while preserving Review492 as the latest
...
- Review240 Effect-First Cleanup, Suspense, And Public API Pins kept the
  post-Review239 fixes Effect-first: best-effort cleanup catches full Causes,
  Start CLI/streaming response/command-runner/starter script lifetime is
  modeled with Effect v4 fibers, React/Solid route Suspense remains isolated at
  host Adapter seams, Solid Resource preload reactivity uses runtime-bound
  Effects, and DB sync/materialization regressions add no Promise APIs.
...
- Review236 Solid Failed Render Cleanup Sequencing kept the follow-up fix
  Effect-first: failed Solid route renders now carry their cleanup Effect back
  to `SolidRouteRenderScopeController`, which chains it through the disposal
  Fiber before later renderer work can proceed.
...
- Review234 Cleanup Effects And Public Surface Pins kept the follow-up fixes
  ...
  `interruptPreloadEffect()`, sync host cleanup conveniences retain immediate
  generation/current-ref ownership where adapters need it, and Query/Devtools/
  route-render public-surface fixes add no Promise APIs.
...
- Review231 kept the post-Review230 fixes Effect-first: Browser Router Kernel
  ...
  React StrictMode-safe runtime and route cleanup uses runtime-bound Effect
  disposal, and public source/Collection namespace fixes changed manifest,
  hover, and type-test ownership without adding Promise surfaces.
- Review230 kept the post-Review229 fixes Effect-first: Resource UI Binding
  teardown now exposes `disposeEffect()` for preload interruption and retained
  ref cleanup, React and Solid adapters run that Effect during hook cleanup,
  React component scopes reject render-time scoped work until commit, and DB
  query/public-surface/documentation fixes changed Type Interfaces and LSP
  ownership without adding Promise surfaces.
...
- Review228 kept the post-Review227 fixes Effect-first: DB query factory,
  ...
  values, Start public file-route resource refs reject Promise-shaped and
  malformed selector results as typed preload failures, and Core helper/docs
  ownership plus the `QueryBuilder` root-export tightening changed only public
  contract shape.
- Review227 kept the post-Review226 fixes Effect-first: Core now owns
  ...
  file-route preload helpers, Capability sync callbacks, and the Start
  diagnostics CLI loader reuse that probe while mapping failures into typed
  Effect boundary errors; direct DB root LSP/type-test ownership and stale
  diagnostics Vite lifetime docs changed only public contract vocabulary.
- Review226 kept the post-Review225 fixes Effect-first: Query entrypoint hovers
  now name Promise-shaped, Effect-shaped, and other non-builder factory-result
  rejection explicitly without changing the runtime contract, and injected Start
  diagnostics CLI loaders now normalize sync throws, Promise-shaped returns,
  and plain non-Effect returns as typed loader errors inside the existing Effect
  CLI flow.
...
- Review210 closed the post-Review209 Effect boundary findings: `EffectInput`
  ...
  accepts Effect objects through the plain object branch, and `toEffect(...)`
  rechecks successful Effect values at runtime. The same pass added verify
  argv validation, React/Solid DB option Promise negatives, and executable
  Start diagnostics CLI parser pins for all three Effect v4 `graph --verbose`
  shared-flag placements.
...
- Review208 kept RuntimeProvider disposal observers Effect-first by widening
  ...
  negative type-test fixtures as deliberate public-boundary assertions, and
  tightened the Start package dry-run gate so POSIX installed-bin verification
  executes the linked `sunfall-arc-start` executable directly.
- Review207 kept copyable starter entrypoints on the synchronous browser host
  facade while updating DB/starter docs to teach `hydrateFromDocumentEffect(...)`
  first or explicitly name `hydrateFromDocument(...)` as the pre-mount facade
  over that Effect helper.
- Review206 kept the Start CLI bin host boundary synchronous but symlink-safe:
  ...
  `sunfall-arc-start` bin through a temporary package-manager-style symlink.
...
- `packages/core/src/runtime.ts`
  ...
  - `SunfallArcRuntime.provide(...)` now exposes a scoped Effect backed by a named
...
- Review 157 focused verification kept `pnpm audit:effect-first` green over 273
  files after adding nested and computed global Promise extraction checks.
...
- Review 203 focused verification kept `pnpm audit:effect-first` green over
  ...
  hazards.
- Review 204 focused verification kept `pnpm audit:effect-first` green over
  408 package/example/config/script/type-test/docs/generated virtual files
  after tightening React/Solid hook hover-doc policy and adapter-root resource
  alias type pins.
...
- Full `pnpm verify` passed after the explicit Effect-any cleanup: 9 package
  ...
  project-console typecheck, 4 project-console test files / 23 tests,
  project-console build, and leak scan.
- `pnpm --filter @sunfall/arc-solid typecheck` and `pnpm typecheck:types` passed
...
- Full `pnpm verify` passed after the Solid UI wildcard cleanup: 9 package
  ...
  project-console build, and leak scan.
- `pnpm --filter @sunfall/arc-start typecheck`, `pnpm typecheck:types`, and
  `pnpm --filter @sunfall/arc-example-project-console typecheck` passed after
...
- `pnpm --filter @sunfall/arc-start typecheck`,
  ...
  `pnpm --filter @sunfall/arc-solid-db typecheck`, and `pnpm typecheck:types`
  ...
  `SunfallArcRuntime<any, any>` source hits are the core ambient runtime accessors
  ...
  `SunfallArcRuntime<any, ER>` adapter bounds where they infer or erase runtime
...
- `pnpm --filter @sunfall/arc-core typecheck`,
  ...
  `pnpm --filter @sunfall/arc-devtools typecheck`,
  `pnpm --filter @sunfall/arc-solid typecheck`,
  `pnpm --filter @sunfall/arc-solid-db typecheck`,
  `pnpm --filter @sunfall/arc-start typecheck`, and `pnpm typecheck:types`
...
- `pnpm --filter @sunfall/arc-core typecheck`,
  `pnpm --filter @sunfall/arc-db typecheck`,
  `pnpm --filter @sunfall/arc-solid-db typecheck`,
  `pnpm --filter @sunfall/arc-start typecheck`, and `pnpm typecheck:types` passed
...
- `pnpm --filter @sunfall/arc-example-devtools-panel verify`,
  `pnpm --filter @sunfall/arc-example-devtools-extension verify`, and
  `pnpm --filter @sunfall/arc-example-project-console typecheck` passed after
...
- Full `pnpm verify` passed after the small async test wrapper cleanup: 9
  ...
  project-console build, and leak scan.
- `pnpm --filter @sunfall/arc-core typecheck`,
  `pnpm --filter @sunfall/arc-solid typecheck`, and
...
- Full `pnpm verify` passed after the core/Solid async test boundary cleanup:
  ...
  project-console typecheck, 4 project-console test files / 23 tests,
  project-console build, and leak scan.
- `pnpm --filter @sunfall/arc-core typecheck` and
  `pnpm exec vitest run packages/core/test/runtime.test.ts` passed after
  converting Sunfall Arc runtime tests to returned Effect programs. Public
...
- `pnpm --filter @sunfall/arc-start typecheck`, `pnpm typecheck`,
...
- DB Collection runtime work now delegates `Collection.define(...)` load,
  ...
  `pnpm --filter @sunfall/arc-db typecheck`, `pnpm typecheck`, and 7 DB test
...
- Start transport body readers now live in `start-transport-body.ts`: JSON
  ...
  errors.
- Start diagnostics now acquires, uses, and releases its temporary Vite server
  through `Effect.acquireUseRelease(...)`, keeping the `sunfall-arc-start` CLI
  diagnostics path and Vite build gate on the same Effect resource lifetime.
...
- Review 135 tightened the Start fetch adapter Promise-return allowance while
  ...
  files.
- The current full `pnpm verify` passed after Review 492 Route Suspense Runtime
  And Prerender Callback Pins: route Suspense ownership, Start prerender
  callback and stream lifetime work, resource GC timer lifecycle, TSRX
  dev-server policy, docs-site prerender output, and current evidence policy
  stay Effect-first without adding Promise APIs: 11 package builds, workspace
  typecheck, type tests, public API inventory audit, Effect-first audit over
  449 physical and virtual files, 58 root test files / 1223 tests,
  package-level verifies for copyable/source packages, generated starter-suite
  packaging/verifies for basic/react/project-console at 20/25/31 app files with
