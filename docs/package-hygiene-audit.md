...
- The current full `pnpm verify` passed after Review492 with 11 package builds,
  ...
  hygiene gate while adding symlinked Start CLI invalid-subcommand rehearsal,
  command-runner signal formatting coverage, and starter shared-script
  ownership checks. Review239 moved package/starter scripts to the
  signal-aware Effect main-runner seam, packaged local runners with copyable
  starters, and refreshed the Effect-first audit to 415 files.
  Review179
