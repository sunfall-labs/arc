...
- Generated starter artifacts
  - Package dry-runs require each Start starter and the project-console example
    to ship `src/routeTree.gen.ts` and `src/sunfall-arc-start-virtual.d.ts`.
...
- Vite dev route updates
  - `sunfallArcStart(...)` rewrites the generated route definitions file and
...
- Start virtual declaration artifact
  - Review173 extends `pnpm example:pack-dry-run` so the copied
    `@sunfall/arc-start/virtual` declaration Adapter is byte-checked:
