...
The workflow grants `id-token: write`, uses npm 11.10 or newer, and does not enable dependency caching in the release job.
...
3. Dispatch `Publish npm packages` with `dist-tag=alpha` and `dry-run=false`.
