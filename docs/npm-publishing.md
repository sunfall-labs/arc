...
- First-time prerelease publishes may cause npm to create `latest` alongside the requested prerelease tag; the publish script attempts to remove that accidental `latest` tag after confirming the requested tag points at the published version, and reports when npm refuses to remove `latest` until a stable release can replace it.
